                ...
                <tr class="bg-white border-b dark:bg-gray-600 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <td class="px-6 py-4">
                        <form action="{{ route('clientes.destroy', $cliente->id) }}" method="POST" onsubmit="return confirm('Tem certeza que deseja excluir este cliente?');">
                            ...
                            @method('DELETE')
                            <button type="submit" 
                                    class="bg-red-500 text-white p-2 rounded-full hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-300 
                                    {{ $cliente->possui_reserva ? 'opacity-50 cursor-not-allowed' : '' }}" 
                                    {{ $cliente->possui_reserva ? 'disabled' : '' }}>
